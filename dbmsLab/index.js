...
app.get("/listings", isLoggedIn, async(req, res, next) => {
...
app.get("/listings", async(req, res, next) => {
    ...
    connection.query(q, (error, results) => {
        ...
        results.forEach((listing) => {
            listing.image = JSON.parse(listing.image);
        });

        res.json(results); // send JSON to React
    });
});
...
app.get("/listings/:id/edit", isLoggedIn, (req, res) => {
    ...
    try {
        connection.query(q, [id], async(error, results) => {
            ...
            results.forEach(listing => {
                listing.image = JSON.parse(listing.image); // now listing.image.url works
            });
...
app.post("/listings/:id/post", isLoggedIn, (req, res, next) => {
    ...
    const user_id = req.user.id; // already have it from session

    const q = `
        INSERT INTO reviews (rating, comment, user_id, listing_id)
        VALUES (?, ?, ?, ?)
    `;
...
app.put("/listings/:id", isLoggedIn, (req, res) => {
    ...
    // Always wrap the image into JSON
    ...
    connection.query(q, [title, description, imageObj, price, location, country, id], (err, results) => {
        if (err) {
            console.log(err);
...
app.get("/listings/:id", isLoggedIn, (req, res) => {
    let { id } = req.params;
    let q = `SELECT * FROM listings WHERE id = ?`;
    let q2 = `SELECT * FROM reviews where listing_id=?`;
    try {
        connection.query(q, [id], async(error, results) => {
            if (error) throw error;
...
app.post("/book/:listing_id", isLoggedIn, (req, res) => {
    ...
    connection.query(checkAvailabilityQuery, [listing_id], (err, results) => {
        ...
        const bookingQuery = "INSERT INTO mybooking (listing_id, user_id, room_count) VALUES (?, ?, ?)";
...
app.get("/my-bookings", isLoggedIn, (req, res) => {
    const query = "SELECT b.id, l.title, b.room_count, b.booking_date FROM mybooking b JOIN listings l ON b.listing_id = l.id WHERE b.user_id = ?";
